...
def test_record_groups_orders_equal_positions_by_id(
    ...
    """Records sharing a position come back in the same order on every call."""
    records = [
        _create(
            user_admin.identity,
            task,
    ...
    expected = sorted(str(record.id) for record in records)

    task_record = ImporterTask.pid.resolve(task.id)
